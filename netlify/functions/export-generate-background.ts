...
const SIGNED_URL_TTL = 3600; // seconden (1 uur) — voor de opgeslagen audit-URL
...
export default async (req: Request): Promise<Response> => {
  ...
  let token = '', type: 'summary' | 'full' = 'summary', language = 'nl', languageName = 'Nederlands', exportNo = '';
  try {
    ...
    return json({ error: 'bad_request' }, 202);
  }

  // Token-formaat valideren (alfanumeriek) — voorkomt filter-injectie in de .or-query.
  if (!/^[a-zA-Z0-9]{8,128}$/.test(token)) {
    return json({ error: 'invalid_token' }, 202);
  }
  // Client-side dossiernummer valideren (dient als poll-sleutel).
  if (!/^[A-Za-z0-9-]{6,64}$/.test(exportNo)) {
    return json({ error: 'invalid_export_no' }, 202);
  }
  ...
  if (caseErr || !caseRow) {
    // Kan niet synchroon terugkoppelen (background); de poll verloopt vanzelf.
    return json({ error: 'unauthorized' }, 202);
  }
  const role: 'initiator' | 'respondent' = caseRow.initiator_token === token ? 'initiator' : 'respondent';
  const caseId = caseRow.id;

  // Hulpfunctie: resultaat/status wegschrijven zodat de poll (export-status) het kan lezen.
  const resultPath = `${caseId}/${exportNo}.result.json`;
  const writeStatus = async (row: any, resultJson: any) => {
    try {
      await supabase.from('exports').upsert([row], { onConflict: 'export_no' });
    } catch (e: any) { console.error('[export-bg] exports upsert mislukt:', e?.message || e); }
    if (resultJson) {
      try {
        await supabase.storage.from('exports').upload(resultPath, Buffer.from(JSON.stringify(resultJson)), {
          contentType: 'application/json', upsert: true,
        });
      } catch (e: any) { console.error('[export-bg] result.json upload mislukt:', e?.message || e); }
    }
  };
  ...
  await writeStatus({
    ...
    status: 'processing', version: 1, consent_version: CONSENT_VERSION,
  }, null);
...
${transcript}`;
  ...
  // 7. Metadata + canonieke payload + hash (exportNo komt van de client als poll-sleutel)
  ...
  // 8. PDF renderen + resultaat wegschrijven (achtergrond: geen synchrone respons).
  //    De poll-functie (export-status) leest straks status + result.json.
  ...
  try {
    ...
    // Audit-kopie van de PDF in de privé-bucket (mag falen).
    try {
      ...
      console.error('[export-bg] PDF-opslag mislukt (download werkt nog via base64):', e?.message || e);
    }
    ...
    const resultJson = {
      ...
      disclaimer,
      pdf_ready: Boolean(pdfBase64),
      pdf_base64: pdfBase64,
      pdf_stored: Boolean(pdfPath),
    };
    await writeStatus({
      export_no: exportNo, case_id: caseId, requested_by_role: role, type, language,
      status: 'rendered', payload_hash: payloadHash, version: 1,
      consent_version: CONSENT_VERSION, pdf_path: pdfPath,
    }, resultJson);
  } catch (e: any) {
    const msg = e?.message || String(e);
    console.error('[export-bg] PDF-generatie mislukt:', msg);
    await writeStatus({
      export_no: exportNo, case_id: caseId, requested_by_role: role, type, language,
      status: 'failed', payload_hash: payloadHash, version: 1, consent_version: CONSENT_VERSION,
    }, { ok: false, export_no: exportNo, status: 'failed', error: 'pdf_generation_failed' });
  }

  return json({ ok: true, export_no: exportNo, status: 'accepted' }, 202);
};
